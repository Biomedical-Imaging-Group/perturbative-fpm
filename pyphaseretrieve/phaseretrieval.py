...
class Ptychography1d(PhaseRetrievalBase):
    ...
    def get_perturbative_model(self, x_est, method:str):
        if method == 'GradientDescent':
            ...
            return self.get_perturbative_ConjugateGradientDescent_model(x_est= x_est)
...
class FourierPtychography2d(PhaseRetrievalBase):
    def __init__(self, probe, shifts_pair:np.ndarray= None, reconstruct_shape:int= None, n_img:int= 25):
...
class MultiplexedPhaseRetrieval(PhaseRetrievalBase):
    def __init__(self, probe, multiplex_led_mask:np.ndarray, shifts_pair:np.ndarray= None, reconstruct_shape= None):
        """shifts_pair is defined as [v_shifts,h_shifts]"""
        self.probe = probe
        self.probe_shape = probe.shape
