...
class LinOpFFT(BaseLinOp):
    ...
    def apply(self, x):
        return cp.fft.fft(x, norm="ortho")

    def applyT(self, x):
        return cp.fft.ifft(x, norm="ortho")
...
class LinOpRoll(BaseLinOp):
    ...
    def apply(self, x):
        return cp.roll(x, shift=self.shifts, axis=0)
...
class LinOpImag(BaseLinOp):
    ...
    def applyT(self, x):
        return cp.zeros_like(x, dtype= cp.complex128)
...
class RealPartExpandOp(BaseLinOp):
    ...
    def apply(self,x):
        return cp.real(self.LinOp.apply(x[0:self.LinOp.in_shape[0]])) - cp.imag(self.LinOp.apply(x[-self.LinOp.in_shape[0]:]) )

    def applyT(self,x):
        return cp.concatenate((cp.real(self.LinOp.applyT(x)), cp.imag(self.LinOp.applyT(x))), axis=0)
...
class LinOpFFT2(BaseLinOp):
    ...
    def apply(self, x):
        return cp.fft.fft2(x, norm="ortho")

    def applyT(self, x):
        return cp.fft.ifft2(x, norm="ortho")
...
class LinOpRoll2(BaseLinOp):
    ...
    def apply(self, x):
        return cp.roll(x, shift=(self.v_shifts,self.h_shifts), axis=(0,1))
...
class LinOpCrop2(BaseLinOp):
    ...
    def applyT(self, x):
        ...
        if      pad_size == 0:
            ...
            return cp.pad(x ,(int(cp.floor(pad_size/2)), int(cp.ceil(pad_size/2))), mode='constant')
...
class StackLinOp(BaseLinOp):
    def __init__(self, LinOpList):
        ...
        self.out_shape = tuple(cp.sum(cp.array([(linop.out_shape if linop.out_shape>(0,) else self.in_shape)
            for linop in LinOpList]),axis=0))

    def apply(self, x):
        return cp.concatenate(tuple(linop.apply(x) for linop in self.LinOpList), axis=0)

    def applyT(self, x):       
        current_idx = 0
        res = cp.zeros(self.in_shape).astype(cp.complex128)
