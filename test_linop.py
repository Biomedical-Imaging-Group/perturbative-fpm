...
if test_roll:
    ...
    theta[:, :, 2] = shifts.fliplr() * 2. / x.shape[2]
    grid = th.nn.functional.affine_grid(theta, (n_shifts, 1, *x.shape[2:]), align_corners=False).to(th.float32).to(x.device)
    sampled = th.nn.functional.grid_sample(x_, grid, align_corners=False).permute(1, 0, 2, 3)
...
if test_prox:
    ...
    div = 1 + roll.T @ crop.T @ pl.Mul(probe_padded_shifted) @ crop @ roll @ th.ones_like(x_bar)
...
field = A @ x_bar
if test_prox:
    A = pl.SumReduce(1, n_shifts) @ pl.RealPartExpand() @ pl.Mul(2 * field.conj()) @ pl.Ifft2() @ pl.Mul(probe) @ crop @ roll @ pl.Fft2()
    div = 1 + roll.T @ crop.T @ pl.Mul(probe_padded_shifted) @ crop @ roll @ th.ones_like(x_bar)
    xs = pl.Ifft2() @ pl.Mul(1 / div) @ pl.Fft2() @ (x_bar + A.T @ b)
    lhs = (A.T @ A + pl.Id()) @ xs
    rhs = x_bar + A.T @ b
