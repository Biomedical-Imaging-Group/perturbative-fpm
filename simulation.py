...
def led_indices_by_angles(positions: th.Tensor, angle_ranges: th.Tensor) -> list[th.Tensor]:
    sin_theta = positions[:, 0:2] / \
        th.sqrt((positions ** 2).sum(-1, keepdims=True))
...
dpc_indices = [th.Tensor(list(set(angle_ind) & set(bf_indices))).to(th.int64)
...
angle_indices = led_indices_by_angles(
    led_positions, angle_ranges=angle_ranges)
bf_ppr_indices = [th.Tensor(list(set(angle_ind) & set(bf_indices))).to(th.int64)
                  for angle_ind in angle_indices]
...
# DF-PPR-2
radii_ranges = [
    th.Tensor([na * f1, na * f2]) for (f1, f2) in [(1., 2)]
]
...
# DF-PPR-3
radii_ranges = [
    th.Tensor([na * f1, na * f2]) for (f1, f2) in [(1., 1.3), (1.3, 1.7), (1.7, 2.)]
]
df_ppr_indices = led_indices_by_radii(
    led_positions, radii_ranges=radii_ranges)
df_ppr_indices = [th.Tensor(ind).to(th.int64) for ind in df_ppr_indices]
