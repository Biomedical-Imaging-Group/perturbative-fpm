...
class new_custom_dataSet(object):
    ...
    def get_shiftsPairs(self):
        ...
        for led_idx in range(self.total_LED_num):
            shifts_h_list.append(self.led_na_dict[str(
                led_idx)][0] / self.wave_lambda / self.fourier_res + 15)
            shifts_v_list.append(-self.led_na_dict[str(led_idx)]
                                 [1] / self.wave_lambda / self.fourier_res - 15)
    ...
    def select_FPM_image(self, img_num_array, centre: list = [0, 0], full_FOV: bool = False, input_size: int = None) -> np.ndarray:
        print('image loading...')
        if full_FOV:
            h_start = int(self.CAMERA_H_RES//2 +
                          centre[0] - self.camera_size//2)
            v_start = int(self.CAMERA_V_RES//2 -
                          centre[1] - self.camera_size//2)
        ...
        for img_idx in img_num_array:
            single_img = img[img_idx, v_start:v_start+crop_size,
                             h_start:h_start+crop_size] - constant_background
        ...
        for idx_row, img_row in enumerate(spamreader):
            ...
            for idx_col, img_element in enumerate(img_row):
                ...
                if len(num_of_element) == 1:
                    img[idx_row, idx_col] = float(
                        num_of_element[0])*float('1'+power_of_element[0])
                else:
...
class FPM_solver(object):
    ...
    def GD_FPM(self, selectedImg_list: list, n_iter=1, lr=1, amp_based_or_not: bool = True, centre: list = [0, 0], initial_est: np.ndarray = None, alpha=0,
        ...
        reconstruction_res = self.dataset.get_reconstruction_size(
            total_selected_led_idx_array=selectedImg_list)
        ...
        y = cp.array(self.dataset.select_FPM_image(
        ...
        pr_model = phaseretrieval.FourierPtychography2d(
            probe=probe, shifts_pair=total_shifts_pair, reconstruct_shape=reconstruction_shape)
        ...
        if amp_based_or_not:
            ...
            gd_method = algos.GradientDescent(
                pr_model, loss_func=None, line_search=True, acceleration=None)
        x_est = gd_method.iterate(
            y=y, initial_est=initial_est, n_iter=n_iter, lr=lr, alpha=alpha)
        ...
        if amp_based_or_not:
            ...
            file_iter = str(f'intenBased_n_iter={
                            n_iter}_lr=Auto_alpha={alpha}')
        file_end = str('.png')
        ...
        padded_phase_img = np.angle(np.fft.ifft2(np.fft.ifftshift(
        ...
        plt.title(f'Phase: ' + file_iter)
        plt.savefig(file_path + file_header +
                    'Phase: padding 256: ' + file_iter + file_end)
        plt.close()
        ...
        plt.savefig(file_path + file_header +
                    'Phase: with line: ' + file_iter + file_end)
        plt.close()
        ...
        plt.savefig(file_path + file_header +
                    'Phase: Zoom in: ' + file_iter + file_end)
        plt.close()
    ...
    def PPR_FPM(self, selectedImg_list: list, n_iter=1, linear_n_iter=5, lr=1, centre: list = [0, 0], initial_est: np.ndarray = None, alpha=0, _lambda=0,
        ...
        if lr is None:
            file_iter = str(f'n_iter={n_iter}, l_n_iter={
                            linear_n_iter}, lambda={_lambda}')
        else:
            file_iter = str(f'n_iter={n_iter}, l_n_iter={
                            linear_n_iter}, lr={lr},  alpha={alpha}')
        file_end = str('.png')
        ...
        plt.savefig(file_path + file_header +
                    'Phase: filter 0.1% ' + file_iter + file_end)
        plt.close()
        ...
        plt.figure()
        plt.imshow(np.real(
            padded_phase_img[plot_y[0]:plot_y[1], plot_x-y_width:plot_x+y_width]), cmap=cm.Greys_r)
        plt.plot([y_width, y_width], [0, 2*y_width-1], 'r-', lw=1.5)
        plt.colorbar()
        plt.title(f'Phase: Zoom in ' + file_iter)
        plt.savefig(file_path + file_header +
