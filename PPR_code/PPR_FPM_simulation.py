...
class U2OS_cell_dataSet(object):
    ...
    def get_reconstruction_size(
        ...
        multiplex_led_array_mask: np.ndarray = None,
        select_led_index_array: np.ndarray = None,
    ) -> int:
        if bright_field_NA:
    ...
    def get_illumnationNA_by_mulitplex_array(
        self, multiplex_led_array_mask: np.ndarray
    ) -> float:
        led_r_map, _ = self.get_led_r_angle_map()
        ...
        for i_multiplex_array in multiplex_led_array_mask:
            temp_led_r = (
                int(np.max(led_r_array[(i_multiplex_array > 0)]))
                * self.led_pitch
            )
    ...
    def get_led_bool_mask(self) -> np.ndarray:
        ...
        led_h_idx_array = np.linspace(
            -led_ra_size, led_ra_size, self.led_dia_number
        )
        led_v_idx_array = np.linspace(
            -led_ra_size, led_ra_size, self.led_dia_number
        )
    ...
    def get_shiftsMap_shiftsPairs(
        self, return_sinThetaMap_ledMap: bool = False
    ):
        led_r_map, _ = self.get_led_r_angle_map()
    ...
    def get_bright_field_LED_map(self):
        ...
        bright_field_led_index_map = (
            birght_field_led_mask * led_index_map
        ).astype(int)
        return (
            bright_field_led_index_map,
            birght_field_led_mask,
            birght_field_led_bool_mask,
        )

    def get_bright_field_multiplex_led_array_mask(
        self, angle_range: np.ndarray, show_angle_map: bool = False
    ) -> np.ndarray:
    ...
    def get_single_dark_multiplex_led_array_mask(
        ...
        dark_field_radius_range_led_bool_mask[birght_field_led_bool_mask] = (
            False
        )
...
def interpolate_img(img, high_res: int):
    ...
    x_idx_array = np.arange(-np.floor(img_size / 2), np.ceil(img_size / 2), 1)
    high_x_idx_array = np.arange(
        -np.floor(img_size / 2), np.ceil(img_size / 2), img_size / high_res
    )

    inter_img_f = interpolate.interp2d(
        x_idx_array, x_idx_array, img, kind="linear"
    )
...
class DPC_and_darkField_solver(object):
    ...
    def select_simulation_range(
        self, groundtruth_size: int, centre: list = [0, 0]
    ):
        ...
        groundtruth_img = np.array(
            img[v_start : v_start + crop_size, h_start : h_start + crop_size]
        )
        plt.figure()
    ...
    def Real_DPC(
        self,
        bright_field_angle_range: np.ndarray,
        ...
        pr_model = phaseretrieval.MultiplexedPhaseRetrieval(
            probe=probe,
            multiplex_led_mask=DPC_multiplex_led_array_mask,
            shifts_pair=total_shifts_pair,
            reconstruct_shape=reconstruction_shape,
        )
        DPC_y = pr_model.apply_ModularSquare(
            np.fft.fft2(simu_img, norm="ortho")
        )
        ...
        croplinop = pl.LinOpCrop2(
            (groundtruth_size, groundtruth_size), reconstruction_shape
        )
        down_sample_GT = croplinop.apply(
            np.fft.fftshift(np.fft.fft2(simu_img, norm="ortho"))
        )
        down_sample_GT_phase = np.angle(
            np.fft.ifft2(np.fft.ifftshift(down_sample_GT), norm="ortho")
        )
        crop_phase_FT_ground = np.abs(
            np.fft.fftshift(np.fft.fft2(down_sample_GT_phase, norm="ortho"))
        )

        phase_FT_xest = np.abs(
            (np.fft.fftshift(np.fft.fft2(np.real(x_est_phase), norm="ortho")))
        )
        error_map = np.abs(
            (phase_FT_xest - crop_phase_FT_ground) / (crop_phase_FT_ground)
        )
        ...
        error_map = np.pad(
            ...
            (
                (
                    int(
                        np.floor(
                            (groundtruth_size - reconstruction_shape[0]) / 2
                        )
                    ),
                    int(
                        np.ceil(
        ...
        phase_FT_ground = np.abs(
            np.fft.fftshift(np.fft.fft2(np.angle(simu_img), norm="ortho"))
        )
        expand_phase_FT_xest = croplinop.applyT(phase_FT_xest)
        noise_map = expand_phase_FT_xest - phase_FT_ground
        mean_square_error = np.mean(noise_map**2)
        root_mean_square_error = np.sqrt(mean_square_error)
        snr_value = 10 * np.log10(
            np.mean(phase_FT_ground**2) / np.mean((noise_map) ** 2)
        )
        plt.figure()
        plt.imshow(error_map, cmap=cm.Greys_r)
        plt.colorbar()
        plt.title(
            "FT Error map, SNR: {:.2f}, alpha: {:.3f}, RMSE: {:.4f}".format(
                snr_value, alpha, root_mean_square_error
            )
        )
        plt.savefig(
            file_path
        ...
        plt.colorbar()
        plt.title(file_header + f" phase padding, " + file_pattern)
        plt.savefig(
            file_path
            + file_header
            + " phase padding, "
    ...
    def BF_PPR(
        ...
        DPC_y = pr_model.apply_ModularSquare(
            np.fft.fft2(np.array(simu_img), norm="ortho")
        )
        for idx in range(int(DPC_y.shape[0] / self.camera_size)):
            plt.figure()
            plt.imshow(
                DPC_y[
                    idx * self.camera_size : (idx + 1) * self.camera_size, :
                ],
                cmap=cm.Greys_r,
            )
            plt.colorbar()
            ...
            plt.savefig(
                "_recon_img/Simulation_Generated measurement {}.png".format(
                    idx + 1
                )
            )
            plt.close()
        ...
        # initial guess
        initial_est = th.ones(
            shape=reconstruction_shape, dtype=th.complex128
        )
        ...
        if lr is None:
            file_iter = str(
                f"n_iter={n_iter}, l_n_iter={linear_n_iter}, NA_range[0,1]"
            )
        else:
            file_iter = str(
                f"n_iter={n_iter}, l_n_iter={linear_n_iter}, lr={lr}, NA_range[0,1]"
            )
        file_end = str(".png")

        plt.figure()
        plt.imshow(th.abs(x_est) ** 2, cmap=cm.Greys_r)
        plt.colorbar()
        ...
        plt.figure()
        plt.imshow(th.angle(x_est), cmap=cm.Greys_r)
        plt.colorbar()
        ...
        phase_FT_xest = th.abs(
            (th.fft.fftshift(th.fft.fft2(th.angle(x_est), norm="ortho")))
        )
        log_FT_phase_xest = th.log(phase_FT_xest)
        ...
        down_sample_GT = croplinop.apply(
            th.fft.fftshift(th.fft.fft2(simu_img, norm="ortho"))
        )
        down_sample_GT_phase = th.angle(
            th.fft.ifft2(th.fft.ifftshift(down_sample_GT), norm="ortho")
        )
        crop_phase_FT_ground = th.abs(
            th.fft.fftshift(th.fft.fft2(down_sample_GT_phase, norm="ortho"))
        )
        ...
        phase_FT_ground = th.abs(
            th.fft.fftshift(th.fft.fft2(th.angle(simu_img), norm="ortho"))
        )
        expand_phase_FT_xest = croplinop.applyT(phase_FT_xest)
        noise_map = expand_phase_FT_xest - phase_FT_ground
        snr_value = 10 * th.log10(
            th.mean(phase_FT_ground**2) / th.mean((noise_map) ** 2)
        )
        plt.figure()
        plt.imshow(error_map, cmap=cm.Greys_r)
        plt.colorbar()
    ...
    def DF_PPR(
        self,
        bright_field_angle_range: th.ndarray,
        ...
        dark_lr=None,
        centre: list = [0, 0],
        read_exsiting_csv: Optional[str] = None,
        initial_est=None,
        ones_initial_est: bool = True,
        NA_range=[1, 1],
        extra_file_name: str = "",
        for_loop_or_not: bool = False,
    ):
        ...
        # read pupil, shifts in F-space
        probe = th.array(self.dataset.get_pupil_mask())
        total_shifts_pair = self.dataset.total_shifts_pair
        ...
        if ones_initial_est:
            ...
            if initial_est is None:
                if read_exsiting_csv is not None:
                    initial_est = read_complex_str_csv(
                        "/home/kshen/Ptychography_Project/phase-retrieval-library/phase-retrieval-library/dataset/Simulation/"
                        + read_exsiting_csv
                    )
                    initial_est = np.array(initial_est)
                else:
                    initial_est = self.DF_PPR(
                        bright_field_angle_range=bright_field_angle_range,
        ...
        total_y = pr_model.apply_ModularSquare(
            np.fft.fft2(np.array(simu_img), norm="ortho")
        )
        ...
        if dark_lr is None:
            file_iter = str(
                f"n_iter={dark_n_iter}, l_n_iter={dark_linear_n_iter}, NA_range[{NA_range[0]}, {NA_range[1]}]"
            )
        else:
            file_iter = str(
                f"n_iter={dark_n_iter}, l_n_iter={dark_linear_n_iter}, lr={dark_lr}, NA_range[{NA_range[0]}, {NA_range[1]}]"
            )
        file_end = str(".png")
    ...
    def FPM(
        ...
        y = pr_model.apply_ModularSquare(
            np.fft.fft2(simu_img, norm="ortho")
        )
        ...
        if amp_based_or_not:
            ...
            gd_method = algos.GradientDescent(
                pr_model, loss_func=None, line_search=True
            )
        ...
        error_map = np.abs(
            (phase_FT_xest - crop_phase_FT_ground)
            / np.abs(crop_phase_FT_ground)
        )
        error_map[error_map > 1] = 1
        error_map = np.pad(
            error_map,
...
if __name__ == "__main__":
    ...
    x_est = _simulation.DF_PPR(
        ...
        centre=centre,
        initial_est=None,
        ones_initial_est=True,
        read_exsiting_csv=None,
        NA_range=[1, outer_NA_facter],
        extra_file_name="2DC patterns_",
    )
