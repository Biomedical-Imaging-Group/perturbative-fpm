...
class new_custom_dataSet(object):
    ...
    def get_shiftsPairs(self):
        ...
        for led_idx in range(self.total_LED_num):
            shifts_h_list.append(self.led_na_dict[str(led_idx)][0]/ self.wave_lambda / self.fourier_res + 8)
            shifts_v_list.append(self.led_na_dict[str(led_idx)][1]/ self.wave_lambda / self.fourier_res - 8) 
    ...
    def selectImg_by_singleDAT_selectImgList(self, img_num_array, centre:list = [0, 0], full_FOV:bool= False, input_size:int= None) -> np.ndarray:
        ...
        if self.camera_size == 256:
            img_bright  = skio.imread(RESOLUTION256_250_BACKREMOVE_AVERAGE500_PATH)
            img_dark    = skio.imread(RESOLUTION256_250_BACKREMOVE_AVERAGE500_PATH)

        elif self.camera_size == 128:
            img_bright  = skio.imread(RESOLUTION128_250_BACKREMOVE_AVERAGE500_PATH)
            img_dark    = skio.imread(RESOLUTION128_250_BACKREMOVE_AVERAGE500_PATH)
        else:
        ...
        for image_key in img_num_array:
            ...
            plt.savefig(f'_measurement_or_background/measurement {image_key}.png')
            plt.close()
...
class PPR_DPC_solver(object):
    ...
    def Real_DPC(self, selectedImg_list:list,  centre:list = [0,0], alpha= 0, file_pattern= '2 halves',
        ...
        DPC_y                       = np.array(self.dataset.selectImg_by_singleDAT_selectImgList(img_num_array= selectedImg_list, centre= centre))
        ...
        padded_phase_FT  = croplinop.applyT(np.fft.fftshift(np.fft.fft2(x_est_phase, norm= 'ortho')))
        padded_phase_img = np.fft.ifft2(np.fft.ifftshift(padded_phase_FT), norm= 'ortho')* (self.camera_size*2)/reconstruction_shape[0]
        plt.figure()
        plt.imshow(np.real(padded_phase_img), cmap=cm.Greys_r)
        plt.colorbar()
        plt.title(f'Phase padding: '+ file_pattern)
        plt.savefig( file_path + file_header + ' Phase: padding 256, ' + file_alpha + file_pattern + file_end)
        plt.close()
        ...
        plt.savefig( file_path + file_header + ' Phase: filter 0.1% ' + file_alpha + file_pattern + file_end)
        plt.close()
        ...
        plt.savefig(file_path + file_header + ' Phase: filter 0.1% with scale bar ' + file_alpha + file_pattern + file_end)
        plt.close()
        ...
        plt.savefig(file_path + file_header + 'Phase: Log FT image ' + file_alpha + file_pattern + file_end)
        plt.close()
        ...
        plt.savefig( file_path + file_header + ' Phase: with line ' + file_alpha + file_pattern + file_end)
        plt.close()
        ...
        plt.savefig( file_path + file_header + ' Phase: Zoom in, ' + file_alpha + file_pattern + file_end)
        plt.close()
        ...
        plt.savefig( file_path + file_header + ' Phase: Inset Zoom in, ' + file_alpha + file_pattern + file_end)
        plt.close()
        ...
        plt.savefig( file_path + file_header + ' Profile of cross-section, ' + file_alpha + file_pattern + file_end)
        plt.close()
        # =========== END output image ===========
    ...
    def PPR_DPC(self, selectedImg_list:list, n_iter= 1, linear_n_iter= 5, lr= 1, centre:list = [0,0], initial_est:np.ndarray= None,
        ...
        if self.gpu:
            y                           = cp.array(self.dataset.selectImg_by_singleDAT_selectImgList(img_num_array= selectedImg_list, centre= centre))
            probe                       = cp.array(self.dataset.pupil_mask)
        else:
            y                           = np.array(self.dataset.selectImg_by_singleDAT_selectImgList(img_num_array= selectedImg_list, centre= centre))
            probe                       = np.array(self.dataset.pupil_mask)
        total_shifts_pair           = self.dataset.total_shifts_pair
        ...
        if self.gpu:
            ...
            if initial_est is None:
                initial_est             = np.ones(shape= reconstruction_shape, dtype=np.complex128)
            else:
        ...
        if lr is None:
            file_iter   = str(f'n_iter={n_iter}, l_n_iter={linear_n_iter}, lambda = {_lambda}, NA_range[{NA_range[0]}, {NA_range[1]}]')
        else:
        ...
        croplinop = LinOpCrop2(in_shape= (self.camera_size*2,self.camera_size*2), crop_shape= reconstruction_shape )
        img = np.abs((np.fft.fftshift(np.fft.fft2(np.angle(x_est), norm= 'ortho'))))
...
if __name__ == '__main__':
    ...
    ## Real DPC ========================================================
    img_list    = [0,1]
    _PPR_DPC_solver.Real_DPC(selectedImg_list= img_list, alpha= 0.1, centre= centre)

    ## Bright PPR ======================================================
    img_list    = [0,1,2]
    initial_est = _PPR_DPC_solver.PPR_DPC(selectedImg_list= img_list, n_iter= 3, linear_n_iter= 6, lr= None, NA_range=[0,1], centre= centre,
                                            _lambda=0)
    ## Direct Dark PPR ==================================================
    img_list    = [0,1,2,8]
    _PPR_DPC_solver.PPR_DPC(selectedImg_list= img_list, n_iter= 4, linear_n_iter= 5, lr= None, NA_range=[1,2], initial_est= initial_est, centre= centre,
                                                _lambda= 0)

    img_list    = [0,1,2,6,7]
    _PPR_DPC_solver.PPR_DPC(selectedImg_list= img_list, n_iter= 4, linear_n_iter= 5, lr= None, NA_range=[1,1.5], initial_est= initial_est, centre= centre,
                                                _lambda= 0)

    img_list    = [0,1,2,3,4,5]
    _PPR_DPC_solver.PPR_DPC(selectedImg_list= img_list, n_iter= 6, linear_n_iter= 4, lr= None, NA_range=[1.3,1.7], initial_est= initial_est, centre= centre,
                                                _lambda= 0)