...
class DPC_and_darkField_solver(object):
    ...
    def Real_DPC(self, bright_field_angle_range:np.ndarray, groundtruth_size:int,  centre:list = [0,0], 
        ...
        plt.colorbar()
        plt.title('FT Error map, SNR: {:.2f}, alpha: {:.3f}, RMSE: {:.4f}'.format(snr_value, alpha, root_mean_square_error))
        ...
        padded_phase_img = np.fft.ifft2(np.fft.ifftshift(padded_phase_FT), norm= 'ortho')* (groundtruth_size)/reconstruction_shape[0]
        plt.figure()
        plt.imshow(np.real(padded_phase_img), cmap=cm.Greys_r)
        plt.colorbar()
        plt.title(file_header + f' phase padding, ' + file_pattern)
        plt.savefig(file_path + file_header + ' phase padding, ' + file_alpha + f', '+ file_pattern + file_end)
        plt.close()
        # =========== END output image ===========


    def BF_PPR(self, bright_field_angle_range:np.ndarray, groundtruth_size:int, n_iter= 1, linear_n_iter= 1, lr= 5, centre:list = [0,0], for_loop_or_not:bool = False, alpha=0, _lambda=0):
        print('BF-PPR start \n----------------------')
        ...
        if self.gpu:
            ...
            DPC_y                               = pr_model.apply_ModularSquare(np.fft.fft2(np.array(simu_img), norm="ortho"))
    ...
    def DF_PPR(
        ...
        reconstruction_size                = self.dataset.get_reconstruction_size(multiplex_led_array_mask= total_multiplex_led_mask)
        ...
        if ones_initial_est:
            ...
            if initial_est is None:
                if read_exsiting_csv is not None:
                    ...
                    initial_est = self.DF_PPR(bright_field_angle_range= bright_field_angle_range, groundtruth_size= groundtruth_size, n_iter= bright_n_iter, linear_n_iter= bright_linear_n_iter, lr= bright_lr, centre= centre)
        if self.gpu:
        ...
        if self.gpu:
            ...
            total_y     = pr_model.apply_ModularSquare(np.fft.fft2(np.array(simu_img), norm="ortho"))
    ...
    def FPM(self, groundtruth_size, radius:float, n_iter= 5, lr= 1, centre=[0,0],amp_based_or_not:bool = True, NA_range:list= None,for_loop_or_not:bool = False):
        ...
        if self.gpu:
            y                           = pr_model.apply_ModularSquare(np.fft.fft2(cp.array(simu_img), norm="ortho"))
        else:
        ...
        error_map = np.abs((phase_FT_xest - crop_phase_FT_ground)/np.abs(crop_phase_FT_ground))
        error_map[error_map > 1] = 1
        error_map = np.pad(error_map,((int(np.floor((groundtruth_size-reconstruction_shape[0])/2)), int(np.ceil((groundtruth_size-reconstruction_shape[0])/2))), (0, 0)),mode='constant', constant_values=1)
        error_map = np.pad(error_map,((0, 0), (int(np.floor((groundtruth_size-reconstruction_shape[0])/2)), int(np.ceil((groundtruth_size-reconstruction_shape[0])/2)))),mode='constant', constant_values=1)
...
if __name__ == '__main__':
    ...
    _simulation.Real_DPC(bright_field_angle_range= bright_angle_range, groundtruth_size= groundtruth_size, centre= centre, alpha= 0.1
                         , file_pattern = pattern)

    ## 2. BF-PPR ==========================
    ## ====================================
    bright_angle_range = np.array([[0,180],[90,270],[0,361]])
    x_est = _simulation.BF_PPR(bright_field_angle_range= bright_angle_range, groundtruth_size= groundtruth_size, 
                    n_iter= 100, linear_n_iter= 120, centre= centre, lr= None, _lambda= 0, alpha= 0)

    ## 2. DF-PPR =======================
    ## =================================
    bright_angle_range      = np.array([[0,180],[90,270],[0,361]])
    multi_angle_range_list  = [[0,361],
                               [0,361]]
    ...
    multi_radius_range_list = [single_radius_range,
                               second_radius_range]
    ...
    x_est = _simulation.DF_PPR(
        ...
        centre= centre, 
        initial_est= None, ones_initial_est= False, read_exsiting_csv= '3_patterns_bright_PPR_DPC_100x100_csv_file.csv', 
    ...
    outer_NA = _simulation.dataset.NA * outer_NA_facter
    outer_dark_radius       = (outer_NA*_simulation.dataset.led_d_z)/np.sqrt(1-outer_NA**2)/_simulation.dataset.led_pitch
    x_est = _simulation.FPM(groundtruth_size=groundtruth_size, centre=centre, radius= outer_dark_radius, n_iter=20, lr= 1e-2, amp_based_or_not= True, NA_range=[0,2],
