...
class ptycho2d_Laura_dataSet(object):
    ...
    def get_bright_field_LED_map(self):
        ...
        bright_field_led_map = ((led_NA_map <= self.NA) * led_index_map).astype(int)
    ...
    def DPC_sum_image_select(self, img_index_array, remove_background:bool = True):
        file_path       = DAT_FILE_PATH
...
class DPC_test(object):
    ...
    def DPC_img(self, camera_size, n_iter= 1, GD_n_iter= 5,lr= 1):
        ...
        ## 3. LED pattern selection
        LED_idx_array_top                               = bright_LED_array
        LED_number_top                                  = len(LED_idx_array_top)
        y_top, img_list_top, shifts_pair_top            = self.ptycho_data.DPC_sum_image_select(LED_idx_array_top, remove_background= False)
        y_total_top                                     = np.tile(y_top, reps= (LED_number_top,1))

        LED_idx_array_bottom                            = bright_LED_array
        LED_number_bottom                               = len(LED_idx_array_bottom)
        y_bottom, img_list_bottom, shifts_pair_bottom   = self.ptycho_data.DPC_sum_image_select(LED_idx_array_bottom, remove_background= False)
        y_total_bottom                                  = np.tile(y_bottom, reps= (LED_number_bottom,1))

        y                           = np.concatenate((y_total_top,y_total_bottom), axis=0)
        y                           = cp.array(y)

        shifts_pair                 = np.concatenate((shifts_pair_top,shifts_pair_bottom), axis=0)
        img_list_top.extend(img_list_bottom)
...
if __name__ == '__main__':
    ...
    # bright_LED_map      = dataset.get_bright_field_LED_map()
    # bright_LED_array    = bright_LED_map[bright_LED_map>0]
    # print(bright_LED_map)
    # print(bright_LED_array[22:])
    # print(len(bright_LED_array[0:22]))

    DPC_obj = DPC_test()
    # DPC_obj.FPM_img(camera_size= 256, n_iter= 1, GD_n_iter= 100,lr= None)
    # DPC_obj.FPM_img(camera_size= 256, n_iter= 1, GD_n_iter= 200,lr= 1e-3)

    DPC_obj.DPC_img(camera_size= 256, n_iter= 1, GD_n_iter= 100,lr= None)
