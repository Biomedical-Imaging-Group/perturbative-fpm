...
class new_custom_dataSet(object):
    ...
    def selectImg_by_singleDAT_selectImgList(self, img_num_array, centre:list = [0, 0]) -> np.ndarray:
        ...
        for image_key in img_num_array:
            ...
            constant_background = len(self.led_patterns_dict[pattern][image_key]) * np.mean(FPM_img_background) * background_fac
...
class PPR_DPC_solver(object):
    ...
    def Real_DPC(self, selectedImg_list:list,  centre:list = [0,0], alpha= 0, file_pattern= '2 halves',
        ...
        padded_phase_FT  = croplinop.applyT(np.fft.fftshift(np.fft.fft2(np.real(x_est_phase), norm= 'ortho')))
        padded_phase_img = np.real(np.fft.ifft2(np.fft.ifftshift(padded_phase_FT), norm= 'ortho') * (self.camera_size*2)/reconstruction_shape[0])
    ...
    def PPR_DPC(self, selectedImg_list:list, n_iter= 1, linear_n_iter= 5, lr= 1, centre:list = [0,0], initial_est:np.ndarray= None,
        ...
        y                           = cp.array(self.dataset.selectImg_by_singleDAT_selectImgList(img_num_array= selectedImg_list))
