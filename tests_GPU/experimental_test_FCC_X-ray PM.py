...
class FCC_dataset(object):
    def __init__(self, camera_size: int) -> None:
        ...
        self.probe_position = np.array(
            self.mat_data['probe_positions'])  # with unit 27.43nm
        self.measured_intensities = np.array(
            self.mat_data['measured_intensities'])
    ...
    def crop_measured_intensities(self):
        ...
        self.measured_intensities = self.measured_intensities[v_start:v_start +
                                                              self.camera_size, h_start:h_start+self.camera_size, :]

    def crop_detector_mask(self):
        v_size, h_size = self.detector_mask.shape
        v_start = int(v_size//2 - (self.camera_size//2))
        h_start = int(h_size//2 - (self.camera_size//2))
        self.detector_mask = self.detector_mask[v_start:v_start +
                                                self.camera_size, h_start:h_start+self.camera_size]
    ...
    def crop_probe(self):
        ...
        v_start = int(v_size//2 - (self.camera_size//2))
        h_start = int(h_size//2 - (self.camera_size//2))
        self.probe = self.probe[v_start:v_start +
                                self.camera_size, h_start:h_start+self.camera_size]
    ...
    def get_original_pixel_resolution(self):
        f_spatial_max = (self.original_camera_size * self.pixel_pitch) / \
            (self.wave_lambda*self.distance_to_det)
    ...
    def get_pixel_resolution(self):
        f_spatial_max = (self.camera_size * self.pixel_pitch) / \
            (self.wave_lambda*self.distance_to_det)
        pixel_resolution = 1/f_spatial_max
        return pixel_resolution
    ...
    def get_shifts_pairs(self):
        ...
        shifts_pairs = np.concatenate([shifts_v.reshape(
            self.total_probe_n, 1), shifts_h.reshape(self.total_probe_n, 1)], axis=1)
    ...
    def select_images(self, x_upper_um=25, x_lower_um=-25, y_upper_um=15, y_lower_um=-15, scatter_plot: bool = False, remove_background: bool = False, concatenating_image: bool = True):
        img_idx_array = np.linspace(
            0, self.total_probe_n-1, self.total_probe_n)
        cat_imgIDX_shiftsPair = np.concatenate((img_idx_array.reshape(
            self.total_probe_n, 1), self.total_shifts_pair), axis=1)
        ...
        crop_x_upper_mask = (
            self.total_shifts_pair[:, 1]*self.pixel_resolution) < crop_x_upper_boundry
        crop_x_lower_mask = crop_x_lower_boundry < (
            self.total_shifts_pair[:, 1]*self.pixel_resolution)
        ...
        crop_y_upper_mask = (
            self.total_shifts_pair[:, 0]*self.pixel_resolution) < crop_y_upper_boundry
        crop_y_lower_mask = crop_y_lower_boundry < (
            self.total_shifts_pair[:, 0]*self.pixel_resolution)
        ...
        cropped_imgIDX_shiftsPair[:,
                                  1] = cropped_imgIDX_shiftsPair[:, 1] - y_centre
        cropped_imgIDX_shiftsPair[:,
                                  2] = cropped_imgIDX_shiftsPair[:, 2] - x_centre
        shifts_pairs = cropped_imgIDX_shiftsPair[:, 1:3]

        if scatter_plot:
            self.scatter_plot_rendering(x=cropped_imgIDX_shiftsPair[:, 2], y=cropped_imgIDX_shiftsPair[:, 1], title='Shifted Probe Position',
                                        x_label='X shifts [9.9 nm]', y_label='Y Shifts [14.16 nm]', file_name='shifted_probe_position')
            # self.scatter_plot_animation(shifts_pairs)

        reconstruction_x_size = int(
            (np.max(shifts_pairs[:, 1]) - np.min(shifts_pairs[:, 1])))
        reconstruction_y_size = int(
            (np.max(shifts_pairs[:, 0]) - np.min(shifts_pairs[:, 0])))
        ...
        for _, keys in enumerate(cropped_imgIDX_shiftsPair):
            img = cp.array(
                self.measured_intensities[:, :, int(keys[0])]*self.detector_mask)
            img_list.append(img)
    ...
    def mat2img(self):
        for idx in range(self.measured_intensities.shape[2]):
            ...
            plt.savefig(
                '/home/kshen/Ptychography_Project/phase-retrieval-library/phase-retrieval-library/dataset/X_ray_FCC_particle/pic/fcc_id1_{0:04}.png'.format(idx+1))
            plt.close()
    ...
    def scatter_plot_animation(self, scatter_position, title: str = 'Test', x_label: str = 'X label', y_label: str = 'Y label', file_name: str = 'Test_ani'):
        ...
        for idx, position in enumerate(scatter_position):
            ...
            y.append(position[0])
            frames.append(
                [ax.scatter(x, y, s=4, marker="o", edgecolor='black')])
        ax.set_title(title)
        ax.set_xlabel(x_label)
        ax.set_ylabel(y_label)
        print('rendering...')
        ani = animation.ArtistAnimation(
            fig, frames, interval=10, blit=True, repeat_delay=1000)
...
class XrayPM_solver(object):
    ...
    def xrayPM(self, camera_size: int, n_iter: int, lr):
        ...
        y, img_list, shifts_pairs, img_idx, reconstruct_shape = self.ptycho_data.select_images(
            x_upper_um=15, x_lower_um=0, y_upper_um=7.5, y_lower_um=-7.5, scatter_plot=True)
        print(f'recontruction shape: {reconstruct_shape}')

        self.ptycho_2d_model = phaseretrieval.XRay_Ptychography2d(
            probe=probe, shifts_pair=shifts_pairs, reconstruct_shape=reconstruct_shape)

        # 3. solver
        loss_function = loss.loss_amplitude_based(epsilon=1e-4)
        GD_method = algos.GradientDescent(
            self.ptycho_2d_model, loss_func=loss_function, line_search=None, acceleration="conjugate gradient")
        ...
        plt.savefig('_recon_img/Xray_Intensity.png')
