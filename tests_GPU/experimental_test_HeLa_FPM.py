...
class HeLa_cell_dataSet(object):
    ...
    def get_illumnationNA_by_mulitplex_array(self, multiplex_led_array_mask: np.ndarray) -> float:
        ...
        for i_multiplex_array in multiplex_led_array_mask:
            temp_led_r = int(
                np.max(led_r_array[(i_multiplex_array > 0)])) * self.led_pitch
    ...
    def get_led_bool_mask(self) -> np.ndarray:
        ...
        led_h_idx_array = np.linspace(-led_ra_size,
                                      led_ra_size, self.led_dia_number)
        led_v_idx_array = np.linspace(-led_ra_size,
                                      led_ra_size, self.led_dia_number)

        led_h_idx_map, led_v_idx_map = np.meshgrid(
            led_h_idx_array, led_v_idx_array)
    ...
    def get_led_360_coordinate_angle_map(self) -> np.ndarray:
        ...
        led_360_anlge_map = np.flip(
            np.abs((led_360_anlge_map - np.abs(led_360_anlge_map))/2), axis=1)
    ...
    def get_shiftsMap_shiftsPairs(self, return_sinThetaMap_ledMap: bool = False):
        ...
        led_to_sample_dist_map = np.sqrt(
        ...
        if return_sinThetaMap_ledMap:
            ...
            shifts_pair = np.concatenate(
                [shifts_v.reshape(n_used_led, 1), shifts_h.reshape(n_used_led, 1)], axis=1)
            shifts_pair[145, :] = [0, 0]
    ...
    def get_bright_field_LED_map(self):
        sinTheta_h_map, sinTheta_v_map, _ = self.get_shiftsMap_shiftsPairs(
            return_sinThetaMap_ledMap=True)
        ...
        bright_field_led_index_map = (
            birght_field_led_mask * led_index_map).astype(int)
    ...
    def get_bright_field_multiplex_led_array_mask(self, angle_range: np.ndarray, show_angle_map: bool = False) -> np.ndarray:
        ...
        for idx in range(angle_range.shape[0]):
            if (angle_range[idx, 1] - angle_range[idx, 0]) < 0:
                led_angle_bool_mask = np.logical_and(np.logical_or(
                    angle_range[idx, 0] < led_angle_map, led_angle_map < angle_range[idx, 1]), led_r_map != 0)
            else:
                led_angle_bool_mask = np.logical_and(
                    angle_range[idx, 0] < led_angle_map, led_angle_map < angle_range[idx, 1])
            multiplex_led_idx_map = led_index_map*led_bool_mask * \
                birght_field_led_mask*led_angle_bool_mask

            multiplex_led_bool_array_mask = multiplex_led_idx_map[led_bool_mask] > 1
            multiplex_led_array_mask = multiplex_led_bool_array_mask.astype(
                int)
    ...
    def get_single_dark_multiplex_led_array_mask(self, single_angle_range: list, single_radius_range: list) -> np.ndarray:
        ...
        dark_field_radius_range_led_bool_mask = np.logical_and(
            single_radius_range[0] <= led_r_map, led_r_map <= single_radius_range[1])
        ...
        if (single_angle_range[1] - single_angle_range[0]) < 0:
            dark_field_angle_range_led_bool_mask = np.logical_and(np.logical_or(
                single_angle_range[0] < led_angle_map, led_angle_map < single_angle_range[1]), led_r_map != 0)
        else:
            dark_field_angle_range_led_bool_mask = np.logical_and(
                single_angle_range[0] < led_angle_map, led_angle_map < single_angle_range[1])

        led_index_map = self.get_led_index_map()
        led_bool_mask = self.get_led_bool_mask()
        multiplex_led_idx_map = led_index_map*led_bool_mask * \
            dark_field_radius_range_led_bool_mask*dark_field_angle_range_led_bool_mask
        ...
        for _, i_img_array_mask in enumerate(multiplex_led_array_mask):
            ...
            for _idx, _weight in enumerate(i_img_array_mask):
                if _weight > 0:
                    ...
                    crop_img = img[v_start:v_start+crop_size,
                                   h_start:h_start+crop_size]*_weight
                    single_img += crop_img
    ...
    def select_image_by_imgIndex(self, img_index_array, centre: list = [0, 0], remove_background: bool = False, show_background: bool = False):
        ...
        for i in img_index_array:
            ...
            crop_img = img[v_start:v_start +
                           crop_size, h_start:h_start+crop_size]
    ...
    def crop_rendering(self, centre: list = [0, 0]) -> None:
        ...
        rect = patches.Rectangle((h_center-crop_half_size, v_center-crop_half_size),
                                 self.camera_size, self.camera_size, linewidth=2, edgecolor='r', facecolor='none')
        ax.add_patch(rect)
        plt.title('Cropping')
...
class FPM_solver(object):
    ...
    def FPM(self, camera_size, n_iter=5, lr=1, cropping_center=[0, 0], amp_based_or_not: bool = True, for_loop_or_not: bool = False):
        ...
        y, img_list, shifts_pair = self.dataset.select_image_by_imgIndex(
            img_index_array=img_idx_array, centre=cropping_center, remove_background=True)
        y = cp.array(y)
        self.phase_model = phaseretrieval.FourierPtychography2d(
            probe=probe, shifts_pair=shifts_pair, reconstruct_shape=reconstruction_shape)
        ...
        if amp_based_or_not:
            ...
            gd_method = algos.GradientDescent(
                self.phase_model, loss_func=loss_function, line_search=None)
        else:
            gd_method = algos.GradientDescent(
                self.phase_model, loss_func=None, line_search=True)
        ...
        if for_loop_or_not == False:
            ...
            plt.savefig(
                '_recon_img/HeLa_FPM_Intensity: Reconstructed image.png')
            plt.close()
            ...
            plt.savefig(
                '_recon_img/HeLa_FPM_Phase99: Reconstruction image.png')
            plt.close()

            histogram, bin_edges = np.histogram(
                np.angle(x_est.get()), bins=(x_est.shape[0]*x_est.shape[1]))
            plt.figure()
            ...
            _, max_ylim = plt.ylim()
            plt.text(phase_img_99.get()*1.1, max_ylim*0.9,
                     '99.99%= {:.2f}'.format(phase_img_99.get()))
            plt.axvline(phase_img_99.get(), color='k',
            ...
            plt.savefig(
                file_path + f'/n_iter={n_iter}' + file_header + 'Intensity: ' + file_iter)
            plt.close()
            ...
            plt.savefig(
                file_path + f'/n_iter={n_iter}' + file_header + 'Phase: ' + file_iter)
            plt.close()
            ...
            plt.savefig(
                file_path + f'/n_iter={n_iter}' + file_header + 'Phase99: ' + file_iter)
            plt.close()
            ...
            plt.savefig(
                file_path + f'/n_iter={n_iter}' + file_header + 'Phase Histogram: ' + file_iter)
            plt.close()
        return x_est
    ...
    def FPM_PPR(self, camera_size: int, n_iter: int, linear_n_iter, lr, centre: list = [0, 0], img_idx_array: np.ndarray = None, for_loop_or_not: bool = False):
        ...
        probe = cp.array(self.dataset.get_pupil_mask())
        y, img_list, shifts_pair = self.dataset.select_image_by_imgIndex(
            img_index_array=img_index_array, centre=centre)
        y = cp.array(y)
        self.phase_model = phaseretrieval.FourierPtychography2d(
            probe=probe, shifts_pair=shifts_pair, reconstruct_shape=reconstruction_shape)
        ...
        if lr is not None:
            x_est = ppr_method.iterate_GradientDescent(
                y=y, initial_est=initial_est, n_iter=n_iter, linear_n_iter=linear_n_iter, lr=lr)
        else:
            x_est = ppr_method.iterate_ConjugateGradientDescent(
                y=y, initial_est=initial_est, n_iter=n_iter, linear_n_iter=linear_n_iter)
    ...
    def bright_FPM(self, camera_size, n_iter=5, lr=1, cropping_center=[0, 0], amp_based_or_not: bool = True, for_loop_or_not: bool = False):
        ...
        probe = cp.array(self.dataset.get_pupil_mask())
        y, img_list, shifts_pair = self.dataset.select_image_by_imgIndex(
            img_index_array=bright_LED_index_array, centre=cropping_center, remove_background=True)
        y = cp.array(y)
        self.phase_model = phaseretrieval.FourierPtychography2d(
            probe=probe, shifts_pair=shifts_pair, reconstruct_shape=reconstruction_shape)
        ...
        if for_loop_or_not == False:
            ...
            plt.savefig(
                '_recon_img/HeLa_FPM_Bright_Sum Bright Field Image.png')
            plt.close()
            ...
            plt.savefig(
                '_recon_img/HeLa_FPM_Bright_Phase99: Reconstruction image.png')
            plt.close()
            ...
            _, max_ylim = plt.ylim()
            plt.text(phase_img_99.get()*1.1, max_ylim*0.9,
                     '99.995%= {:.2f}'.format(phase_img_99.get()))
            plt.axvline(phase_img_99.get(), color='k',
                        linestyle='dashed', linewidth=1)
...
class DPC_and_darkField_solver(object):
    ...
    def DPC(self, bright_field_angle_range: np.ndarray, n_iter=1, linear_n_iter=5, lr=1, centre: list = [0, 0], for_loop_or_not: bool = False):
        ...
        self.DPC_multiplex_led_array_mask = self.dataset.get_bright_field_multiplex_led_array_mask(
            angle_range=bright_field_angle_range, show_angle_map=True)
        self.DPC_y = self.dataset.select_image_by_multiplexArrayMask(
            multiplex_led_array_mask=self.DPC_multiplex_led_array_mask, centre=centre)
        ...
        pr_model = phaseretrieval.MultiplexedPhaseRetrieval(
            probe=probe, multiplex_led_mask=self.DPC_multiplex_led_array_mask, shifts_pair=total_shifts_pair, reconstruct_shape=reconstruction_shape)
        ppr_method = algos.PerturbativePhase(pr_model)

        if lr is not None:
            x_est = ppr_method.iterate_GradientDescent(
                y=self.DPC_y, initial_est=initial_est, n_iter=n_iter, linear_n_iter=linear_n_iter, lr=lr)
        else:
            x_est = ppr_method.iterate_ConjugateGradientDescent(
                y=self.DPC_y, initial_est=initial_est, n_iter=n_iter, linear_n_iter=linear_n_iter)
        ...
        if for_loop_or_not == False:
            ...
            plt.imshow(np.abs(x_est.get())**2, cmap=cm.Greys_r)
            plt.colorbar()
            plt.title(f'Intensity GD: linear_n_iter={
                      linear_n_iter}, n_iter={n_iter}, lr={lr}')
            plt.savefig(f'_dpc_img/top_bottom_right_left/GD/n_iter={n_iter}/Intensity GD n_iter={
                        n_iter}, linear_n_iter={linear_n_iter}, lr={lr}.png')
            plt.close()

            plt.figure()
            plt.imshow(np.angle(x_est.get()), cmap=cm.Greys_r)
            plt.colorbar()
            plt.title(f'Phase GD: linear_n_iter={
                      linear_n_iter}, n_iter={n_iter}, lr={lr}')
            plt.savefig(f'_dpc_img/top_bottom_right_left/GD/n_iter={n_iter}//Phase GD n_iter={
                        n_iter}, linear_n_iter={linear_n_iter}, lr={lr}.png')
            plt.close()
        return x_est
    ...
    def dark_field_with_DPC(
        ...
        initial_est = self.DPC(bright_field_angle_range=bright_field_angle_range,
                               n_iter=bright_n_iter, linear_n_iter=bright_linear_n_iter, lr=bright_lr, centre=centre)
        initial_est = np.fft.fft2(initial_est, norm="ortho")
        ...
        self.dark_y = self.dataset.select_image_by_multiplexArrayMask(
            multiplex_led_array_mask=self.dark_multiplex_led_array_mask, centre=centre)
        ...
        total_multiplex_led_mask = np.concatenate(
            (self.DPC_multiplex_led_array_mask, self.dark_multiplex_led_array_mask), axis=0)

        reconstruction_size = self.dataset.get_reconstruction_size(
            multiplex_led_array_mask=self.dark_multiplex_led_array_mask)
        ...
        pr_model = phaseretrieval.MultiplexedPhaseRetrieval(
            probe=probe, multiplex_led_mask=total_multiplex_led_mask, shifts_pair=total_shifts_pair, reconstruct_shape=reconstruction_shape)
        ppr_method = algos.PerturbativePhase(pr_model)

        if dark_lr is not None:
            x_est = ppr_method.iterate_GradientDescent(
                y=total_y, initial_est=initial_est, n_iter=dark_n_iter, linear_n_iter=dark_linear_n_iter, lr=dark_lr)
        else:
            x_est = ppr_method.iterate_ConjugateGradientDescent(
                y=total_y, initial_est=initial_est, n_iter=dark_n_iter, linear_n_iter=dark_linear_n_iter)
...
def interpolate_img(img, high_res: int):
    ...
    high_x_idx_array = np.arange(-math.floor(img_size/2),
                                 math.ceil(img_size/2), img_size/high_res)

    inter_img_f = interpolate.interp2d(
        x_idx_array, x_idx_array, img.get(), kind='linear')
...
if __name__ == '__main__':
    ...
    single_radius_range = [
        np.sqrt(2*(inner_dark_radius**2)), np.sqrt(2*(outer_dark_radius**2))]

    multi_angle_range_list = [[0, 180], [180, 360], [270, 90], [90, 270]]
    multi_radius_range_list = [
        single_radius_range, single_radius_range, single_radius_range, single_radius_range]
