...
class ptycho2d_Laura_dataSet(object):
    ...
    def get_reconstruction_size(self) -> int:
        ...
        reconstruct_dia_number = math.ceil(
    ...
    def get_shiftsMap_shiftsPairs(
        self, return_sinThetaMap_ledMask: bool = False
    ):
        ...
        shifts_h_map = np.round(
            sinTheta_h_map * led_r_mask / self.wave_lambda / self.fourier_res
        )
        shifts_v_map = np.round(
            sinTheta_v_map * led_r_mask / self.wave_lambda / self.fourier_res
        )

        shifts_h = shifts_h_map[led_r_mask == 1]
        shifts_v = shifts_v_map[led_r_mask == 1]
        shifts_pair = np.concatenate(
            [shifts_v.reshape(n_used_led, 1), shifts_h.reshape(n_used_led, 1)],
            axis=1,
        )
    ...
    def get_bright_field_LED_map(self):
        sinTheta_h_map, sinTheta_v_map, led_r_mask = (
            self.get_shiftsMap_shiftsPairs(return_sinThetaMap_ledMask=True)
        )
        ...
        bright_field_led_map = (
            (led_NA_map <= 0.99 * self.NA) * led_index_map
        ).astype(int)
    ...
    def crop_rendering(self, centre: list = [0, 0]) -> None:
        ...
        rect = patches.Rectangle(
            (h_center - crop_half_size, v_center - crop_half_size),
            self.camera_size,
            self.camera_size,
            linewidth=2,
            edgecolor="r",
...
class test_GD_in_ptych2d(object):
    ...
    def generate_rand2d_x(self, in_dim, scale=0.5):
        x = np.random.randn(in_dim, in_dim) + 1j * np.random.randn(
            in_dim, in_dim
        )
        return x
    ...
    def model_test(self, camera_size: int, img_idx_array, n_iter: int, lr):
        ...
        probe = np.array(self.ptycho_data.get_pupil_mask())
        _, _, shifts_pair = self.ptycho_data.select_image(
            img_idx_array, load_img_or_not=False
        )
        self.ptycho_2d_model = phaseretrieval.FourierPtychography2d(
            probe=probe,
            shifts_pair=shifts_pair,
            reconstruct_size=reconstruction_res,
        )
        ...
        initial_est = Spec_method.iterate(y=y)
        ...
        print(
            np.abs(
                (_x_est.T.conj() @ _x)
                / (np.linalg.norm(_x_est) * np.linalg.norm(_x))
            )
        )
    ...
    def real_data_test(
        ...
        probe = np.array(self.ptycho_data.get_pupil_mask())
        y, img_list, shifts_pair = self.ptycho_data.select_image(
            img_idx_array, centre=centre
        )
        self.ptycho_2d_model = phaseretrieval.FourierPtychography2d(
            probe=probe,
            shifts_pair=shifts_pair,
        ...
        ## 6. solve the problem
        initial_est = np.ones(
            shape=(reconstruction_res, reconstruction_res), dtype=np.complex128
        )
        initial_est = np.fft.fft2(initial_est, norm="ortho")
    ...
    def model_test_withautoshifts(
        self, camera_size: int, n_img: int, n_iter: int, lr
    ) -> None:
        ...
        probe = np.array(self.ptycho_data.get_pupil_mask())
        self.ptycho_2d_model = phaseretrieval.FourierPtychography2d(
            probe=probe, reconstruct_size=reconstruction_res, n_img=n_img
        )
        ...
        print(
            np.abs(
                (_x_init.T.conj() @ _x)
                / (np.linalg.norm(_x_init) * np.linalg.norm(_x))
            )
        )
...
class test_PPR_in_ptych2d(object):
    ...
    def model_test(
        self, camera_size: int, img_idx_array, n_iter: int, GD_n_iter: int, lr
    ):
        print("Model test \n----------------------")
        ## 1. use experimental setup from Laura dataset
        self.ptycho_data = ptycho2d_Laura_dataSet(camera_size)
    ...
    def model_test_withautoshifts(
        self, camera_size: int, n_img: int, n_iter: int, GD_n_iter: int, lr
    ) -> None:
        print("Model test without shifts assignment \n----------------------")
        ## 1. use experimental setup from Laura dataset
        self.ptycho_data = ptycho2d_Laura_dataSet(camera_size)
...
def interpolate_img(img, high_res: int):
    ...
    x_idx_array = np.arange(
        -math.floor(img_size / 2), math.ceil(img_size / 2), 1
    )
    high_x_idx_array = np.arange(
        -math.floor(img_size / 2), math.ceil(img_size / 2), img_size / high_res
    )
...
def animation_show(img_list: list, data_range=None):
    ...
    for idx in range(number_of_img):
        frames.append(
            [plt.imshow(img_list[idx], cmap=cm.Greys_r, animated=True)]
        )
    print("rendering...")
    ani = animation.ArtistAnimation(
        fig, frames, interval=200, blit=True, repeat_delay=10000
    )
    if data_range:
...
if __name__ == "__main__":
    ...
    ptycho2d_test.model_test(
        camera_size=50, img_idx_array=img_idx_array, n_iter=500, lr=0.1
    )
