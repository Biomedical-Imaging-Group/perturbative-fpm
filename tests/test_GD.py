...
class GD_algos_test():
    ...
    def test_rand1d_case(self) -> None:
        ...
        in_dim = 800
        out_dim = 16000
        linop = LinOpMatrix(np.random.randn(out_dim, in_dim) +
        ...
        print('model: in_dim: {}, out_dim: {}, oversampling ratio: {}'.format(
            in_dim, out_dim, out_dim/in_dim))

        # 2. GD method solver
        rand_GD_method = algos.GradientDescent(
            pr_model, line_search=True, acceleration=None)
        x_est = rand_GD_method.iterate(y=y, initial_est=None, n_iter=500)

        # 3. print final correlation
        print("Result correlation:")
        print(np.abs((x_est.T.conj() @ x) /
    ...
    def test_Ptychography1d_case_without_spectral(self):
        ...
        probe = (np.ones(in_dim,).astype(np.complex128)
                 * (sampling_grid < ptycho_radius))
        ...
        GD_method = algos.GradientDescent(
            pr_model, line_search=True, acceleration="conjugate gradient")
