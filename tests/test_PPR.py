...
class PPR_algos_test(object):
    ...
    def test_rand1d_case(self) -> None:
        ...
        in_dim = 100
        out_dim = 800
        op_matrix = linop.LinOpMatrix(
            np.random.randn(out_dim, in_dim)
            + 1j * np.random.randn(out_dim, in_dim)
        )
        ...
        print(
            "model: in_dim: {}, out_dim: {}, oversampling ratio: {}".format(
        ...
        x_est = ppr_model.iterate_GradientDescent(
            y=y, n_iter=15, linear_n_iter=25, lr=1e-7
        )
        ...
        print(
            np.abs(
                (x_est.T.conj() @ x)
                / (np.linalg.norm(x_est) * np.linalg.norm(x))
            )
        )
    ...
    def test_Ptychography1d_case_without_spectral(self):
        ...
        # 2.Probe create
        ptycho_radius = 200
        sampling_grid = np.abs(
            np.linspace(-int(np.ceil(in_dim / 2)), in_dim // 2, in_dim)
        )
        probe = np.ones(
            in_dim,
        ).astype(np.complex128) * (sampling_grid < ptycho_radius)
        ...
        # 4. solver
        ppr_model = algos.PerturbativePhase(pr_model)
        x_est = ppr_model.iterate_ConjugateGradientDescent(
            y=y, n_iter=25, linear_n_iter=15
        )
    ...
    def test_Ptychography1d_case_with_spectral(self):
        ...
        ptycho_radius = 250
        sampling_grid = np.abs(
            np.linspace(-in_dim // 2, int(np.ceil(in_dim / 2)) - 1, in_dim)
        )
        probe = np.ones(
            in_dim,
        ).astype(np.complex128) * (sampling_grid < ptycho_radius)
        ...
        x_est = ppr_model.iterate_ConjugateGradientDescent(
            y=y, n_iter=25, linear_n_iter=20
        )
