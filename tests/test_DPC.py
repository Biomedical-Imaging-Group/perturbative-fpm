...
class ptycho2d_Laura_dataSet(object):
    ...
    def multiplex_image_select(self, multiplex_led_array:np.ndarray):      
...
class DPC_test(object):
    ...
    def DPC_img(self, camera_size, angle_range:np.ndarray, n_iter= 1, GD_n_iter= 5,lr= 1):
        ...
        y                               = self.ptycho_data.multiplex_image_select(multiplex_led_array= multiplex_led_array)
        ...
        pr_model                = phaseretrieval.MultiplexedPhaseRetrieval(probe= probe,multiplex_led_mask= multiplex_led_array, shifts_pair= total_shifts_pair, reconstruction_shape= reconstruction_shape)
